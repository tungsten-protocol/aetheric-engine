...
impl<S: SceneKey, A: Action> GlobalSystems<S, A> {
    ...
    ///
    /// Processes input events, updates active scenes, and handles scene
    /// transitions. Called by CoreSystemsOrchestrator each tick.
    ///
    /// # Processing Pipeline
    ///
    /// 1. **Input Processing**: Converts platform events to input state
    /// 2. **Scene Update**: Updates all active scenes with current context
    /// 3. **Transition Processing**: Applies queued scene transitions
    ///
    /// # Arguments
    ///
    /// * `context` - Shared context containing input state, events, and transition queue
    pub(crate) fn update(&mut self, context: &mut GlobalContext<S>) {
        // 1. Process input events into state
        self.input.process_frame(
            &mut context.input_state,
            &context.frame_events
        );

        // 2. Update active scenes
        self.scene_manager.update(context);

        // 3. Process scene transitions
        self.scene_manager.process_transitions(context);

        // Clear frame events after processing
        context.frame_events.clear();
    }
}
